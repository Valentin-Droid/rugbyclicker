...
export function GameProvider({ children }) {
	...
	const loadGameState = useCallback(async (partieId) => {
		...
		const bonusProduction = data.ameliorations.reduce((acc, a) => {
			...
			if (
				type === "multiplicateur_production" ||
				type === "multiplicateur_global"
			) {
				return acc * mult;
			}
			return acc;
		...
		const pps = data.infrastructures.reduce(
			(sum, infra) =>
				sum +
				(infra.quantite || 0) * infra.production_base * (infra.niveau || 1),
			0,
		);
	...
	const tick = useCallback(() => {
		if (!partie) return;
		// L'incrément local d'argent et fans est géré par le backend via sync toutes les 30s.
		// On se contente d'un tick visuel léger pour les animations locales.
		setRessources((prev) =>
			prev.map((r) => {
				if (r.id_ressource === 1) {
	...
	const acheterInfrastructure = useCallback(
		async (infraId) => {
			if (!partie) return;
			const res = await gameService.acheterInfrastructure(
				partie.id_partie,
				infraId,
			);
	...
	const upgraderInfrastructure = useCallback(
		async (infraId) => {
			if (!partie) return;
			const res = await gameService.upgraderInfrastructure(
				partie.id_partie,
				infraId,
			);
			await loadGameState(partie.id_partie);
	...
	const acheterAmelioration = useCallback(
		async (amelId) => {
			if (!partie) return;
			const res = await gameService.acheterAmelioration(
				partie.id_partie,
				amelId,
			);
			await loadGameState(partie.id_partie);
