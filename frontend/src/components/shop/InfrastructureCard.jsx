...
function InfrastructureCard({ infrastructure, isCheapest, isBarelyAffordable }) {
  const { ressources, acheterInfrastructure, upgraderInfrastructure, productionParSeconde } = useGame();
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      if (iconRef.current) {
        ...
        setTimeout(() => {
          if (iconRef.current) iconRef.current.classList.remove('infra-card__icon--bounce');
        }, 500);
      }
  ...
  if (!canBuy) {
    ...
    tooltipLines.push(`Production actuelle : ${formatNumber(productionActuelle)}/s`);
    ...
    >
      <div className="infra-card__header">
        <h4 className="infra-card__name">
          <span ref={iconRef} className="infra-card__icon">{icon}</span>
          {infrastructure.nom}
        </h4>
      ...
      {hoverPreview && quantite > 0 && (
        <p className="infra-card__preview">
          Production passera de {formatNumber(productionActuelle)}/s à {formatNumber(productionApresAchat)}/s
        </p>
      )}
      ...
      {quantite >= 1 && (
        <p className="infra-card__upgrade-preview">
          ...
          ({formatNumber(productionActuelle)}/s → {formatNumber(productionApresUpgrade)}/s)
        </p>
      )}
