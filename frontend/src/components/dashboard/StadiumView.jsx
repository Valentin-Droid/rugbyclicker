...
function StadiumView() {
	...
	const ppsRef = useRef(productionParSeconde);
	...
	useEffect(() => {
		ppsRef.current = productionParSeconde;
	}, [productionParSeconde]);

	useEffect(() => {
		if (ppsRef.current <= 0) return;
		...
		const interval = setInterval(() => {
			...
			setAutoFloats((prev) => {
				const next = [
					...prev,
					{ id, text: `+${pps.toFixed(1)}€`, dir: direction },
				];
	...
	}, []); // L'interval est stable, ppsRef.current est lu à chaque tick
	...
	return (
		<div
			className={`stadium-view${levelUpPulse ? " stadium--levelup-pulse" : ""}`}
		>
			<div className={`stadium-container stadium--stage-${stage}`}>
				...
				{/* ── GIANT PPS COUNTER ────────────────────── */}
				<div
					className={`stadium-pps${ppsUpdated ? " stadium-pps--flash" : ""}`}
				>
				...
				{/* ── LAST ACTION ───────────────────────────── */}
				<div className="stadium-last-action">{lastAction || "\u00A0"}</div>
			</div>
		</div>
	);
}
