...
import { CoinIcon, PeopleIcon, StarIcon } from "../common/GameIcons";
import { formatNumber } from "../../utils/format";

const ICONS = {
	1: <CoinIcon size={16} />,
	2: <PeopleIcon size={16} />,
	3: <StarIcon size={16} />,
};
...
function ResourceBar() {
	...
	useEffect(() => {
		...
		ressources.forEach((r) => {
			...
			if (prev !== undefined && curr !== prev) {
				const flashClass =
					curr > prev ? "resource-card--gain" : "resource-card--loss";
	...
	// Trend detection
	const ppsNow = productionParSeconde;
	const ppsPrev = prevPps.current;
	const trend =
		ppsNow > ppsPrev
			? "resource-trend--up"
			: ppsNow < ppsPrev
	...
	const cheapestUpgradeCost = infrastructures
		...
		.reduce((min, i) => {
			...
			const n = i.niveau || 1;
			const cost = Math.floor(coutBase * 1.15 ** q * 1.5 ** n);
			return cost < min ? cost : min;
		}, Infinity);
