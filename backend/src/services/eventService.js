...
const eventService = {
	 ...
	 */
	applyEvent: async (partieId, eventType) => {
		...
		try {
			...
			// Lire le niveau depuis la DB — ne jamais faire confiance au client
			const partieResult = await client.query(
				"SELECT niveau FROM partie WHERE id_partie = $1",
				[partieId],
			);
			const level = partieResult.rows[0]?.niveau || 1;
